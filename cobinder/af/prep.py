...
from cobinder.af.alphafold.data import pipeline, prep_inputs
from cobinder.af.alphafold.common import protein, residue_constants
from cobinder.af.alphafold.model.tf import shape_placeholders
from cobinder.af.alphafold.model import config


from cobinder.shared.protein import _np_get_cb, pdb_to_string
from cobinder.shared.prep import prep_pos
from cobinder.shared.utils import copy_dict
from cobinder.shared.model import order_aa
...
class _af_prep:
  ...
  def __prep_binder(self, pdb_filename, chain="A",
                   binder_len=50, binder_chain=None,binder_mask=None,
    ...
    _pdb = prep_pdb(pdb_filename, chain=chains, ignore_missing=im)
    res_idx = _pdb["residue_index"]

    if redesign:
      self._target_len = sum([(_pdb["idx"]["chain"] == c).sum() for c in chain.split(",")])
      self._binder_len = self._len = sum([(_pdb["idx"]["chain"] == c).sum() for c in binder_chain.split(",")])
    else:
      self._target_len = _pdb["residue_index"].shape[0]
    ...
    if hotspot is not None:
      self.opt["hotspot"] = prep_pos(hotspot, **_pdb["idx"])["pos"]

    if redesign:
      # binder redesign
      _wt_aatype = _pdb["batch"]["aatype"][self._target_len:]
      self.opt["weights"].update({"dgram_cce":1.0, "rmsd":0.0, "fape":0.0,
                                  "con":0.0, "i_con":0.0, "i_pae":0.0})
    else:
      # binder hallucination
      # 通過填充，將輸入變成矩陣[N_target x N_binder ]
      _pdb["batch"] = make_fixed_size(_pdb["batch"], num_res=sum(self._lengths))
      self.opt["weights"].update({"plddt":0.1, "con":0.0, "i_con":1.0, "i_pae":0.0})

    # configure input features
    inputs = self._prep_features(num_res=sum(self._lengths), num_seq=1) # 生成空白的矩陣，長度為N_target+N_binder
    inputs["residue_index"] = res_idx # 殘基序列
    inputs["batch"] = _pdb["batch"]  # 将pdb中氨基酸的信息放入其中
    inputs.update(get_multi_id(self._lengths)) # 生成 [[0] *  N_target,[1] * N_binder]
    ...
    for n,x in rm_opt.items():
      ...
      for m,y in x.items():
        if isinstance(y,str):
          rm[n][prep_pos(y,**_pdb["idx"])["pos"]] = True
        else:
  ...
  def _prep_binder(self, pdbs, chains,sample_weight=None,
                   binder_len=50, binder_chain=None,binder_mask=None,
                   rm_template_ic=False,
                   use_binder_template=False, 
                   rm_binder_seq=True, rm_binder_sc=True,
    ...
    '''
    pdbs: 输入多个peptide结构
    chains: 输入多个peptide的chain信息
    sample_weight: 每个peptide的loss权重负优化则选择负数
    '''
    ...
    if sample_weight == None:
      self.opt['sample_weight'] = [ 1 ] * len(pdbs)
    else:
      self.opt['sample_weight'] = sample_weight
    self._inputs = []
    for pdb,chain in zip(pdbs,chains):      
      self._inputs.append(self.__prep_binder(pdb,chain,binder_len, binder_chain,binder_mask,
                   rm_template_ic,use_binder_template, rm_binder_seq, rm_binder_sc,rm_target_seq,rm_target_sc,
                   hotspot, ignore_missing, **kwargs)[0])
    self.num_target = len(pdbs)
    self._prep_model(**kwargs)
