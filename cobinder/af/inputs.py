...
from cobinder.shared.model import soft_seq
from cobinder.af.alphafold.common import residue_constants
from cobinder.af.alphafold.model import model, config
...
class _af_inputs:
  ...
  def _get_seq(self, inputs, aux, key):
    ...
    if self.protocol == "binder" or self.protocol == "binder2":
      # concatenate target and binder sequence
      seq_target = jax.nn.one_hot(inputs["batch"]["aatype"][:self._target_len],20) # 转化氨基酸序列成onehot编码[N_target x 20 ](人体有20种氨基酸)
