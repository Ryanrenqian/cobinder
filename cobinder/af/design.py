...
import numpy as np
from cobinder.af.alphafold.common import residue_constants
from cobinder.shared.utils import copy_dict, update_dict, Key, dict_to_str, to_float, softmax, categorical, to_list, copy_missing
...
class _af_design:
  ...
  def restart(self, seed=None, opt=None, weights=None,
              seq=None, mode=None, keep_history=False, reset_opt=True,binder_mask=None, **kwargs):   
    '''
    ...
    if not keep_history:
      ...
      for i in range(self.num_target):
        self._tmp[f'log_{i}'] = []
        self._tmp[f"traj_{i}"] = {"seq":[],"xyz":[],"plddt":[],"pae":[]}
        self._tmp["best"] = {}
    ...
    # initialize sequence
    self.set_seed(seed)
    self.set_seq(seq=seq, mode=mode, **kwargs) # 初始化binder序列
  ...
  def run(self, num_recycles=None, num_models=None, sample_models=None, models=None,
    ...
    for i in range(self.num_target):
      self.aux[f'aux_{i}']["atom_positions"] = auxs[f'aux_{i}']["atom_positions"][0]
      self.aux[f'aux_{i}']["all"] = auxs[f'aux_{i}']
      
      # post-design callbacks
      for fn in (self._callbacks["design"]["post"] + to_list(callback)): fn(self)

      # update log
      self.aux[f'aux_{i}']["log"] = {**self.aux[f'aux_{i}']["losses"]}
      self.aux[f'aux_{i}']["log"]["plddt"] = 1 - self.aux[f'aux_{i}']["log"]["plddt"]
      for k in ["loss","i_ptm","ptm"]: self.aux[f'aux_{i}']["log"][k] = self.aux[f'aux_{i}'][k]
      for k in ["hard","soft","temp"]: self.aux[f'aux_{i}']["log"][k] = self.opt[k]

      # compute sequence recovery
      if self.protocol in ["fixbb","partial"] or (self.protocol in ["binder"] and self._args["redesign"]):
        if self.protocol == "partial":
          aatype = self.aux[f'aux_{i}']["aatype"][...,self.opt["pos"]]
        else:
          aatype = self.aux[f'aux_{i}']["seq"]["pseudo"].argmax(-1)

        mask = self._wt_aatype != -1
        true = self._wt_aatype[mask]
        pred = aatype[...,mask]
        self.aux[f'aux_{i}']["log"]["seqid"] = (true == pred).mean()

      self.aux[f'aux_{i}']["log"] = to_float(self.aux[f'aux_{i}']["log"])
      self.aux[f'aux_{i}']["log"].update({"recycles":int(self.aux["num_recycles"]),
                              "models":model_nums})
  ...
  def _single(self, model_params, backprop=True):
    ...
    if isinstance(self._inputs, list):
      grad = []
      for i,_inputs in enumerate(self._inputs):
        _inputs["opt"] = self.opt
        flags  = [param, model_params, _inputs, self.key()]
        if backprop:
          (loss, _aux), _grad = jax.tree_map(lambda x: x.block_until_ready(), self._model["grad_fn"](*flags))
          # _grad = self.input['mask']
        else:
          loss, _aux = jax.tree_map(lambda x: x.block_until_ready(), self._model["_fn"](*flags))
          _grad = jax.tree_map(np.zeros_like, param)
        _aux.update({"loss":loss,"grad":_grad})
        aux[f'aux_{i}'] = _aux
    else:
      self._inputs["opt"] = self.opt
      flags  = [param, model_params, self._inputs, self.key()]
      if backprop:
        (loss, _aux), grad = self._model["grad_fn"](*flags)
      else:
        loss, _aux = self._model["fn"](*flags)
        grad = jax.tree_map(np.zeros_like, self._params)
      _aux.update({"loss":loss,"grad":grad})
      aux[f'aux_{i}'] = _aux
    return aux
    ...
    if mode in ["backprop","add_prev"]:
      ...
      if isinstance(self._inputs, list):
        for _input in self._inputs:
          L = _input["residue_index"].shape[0]
          
          # intialize previous
          _input["prev"] = {'prev_msa_first_row': np.zeros([L,256]),
      ...
      # 只計算部分recycles的loss，mask recycles的梯度進行忽略
      grad = []
      for m in mask:
        if m == 0:
        ...
        for i,(_input,weight) in enumerate(zip(self._inputs,self.opt['sample_weight'])):
          _aux = aux[f'aux_{i}']
          _input["prev"] = _aux["prev"]
          if m != 0:
            grad.append(jax.tree_map(lambda x:x*m*weight, _aux["grad"]))
        aux["num_recycles"] = num_recycles
        aux["grad"] = jax.tree_map(lambda *x: np.stack(x).sum(0), *grad) # 将所有的梯度求和并放入第一个aux中
    return aux
  ...
  def _print_log(self, print_str=None, aux=None):
    ...
    for i in range(self.num_target):
      _aux = aux[f'aux_{i}']
      if "i_ptm" in _aux["log"]:
        if len(self._lengths) > 1:
          keys.append(f"i_ptm")
        else:
          _aux["log"].pop("i_ptm")

      print(dict_to_str(_aux["log"], filt=self.opt["weights"],
  ...
  def _save_results(self, aux=None, save_best=False,
    ...
    for i in range(self.num_target):
      _aux = aux[f'aux_{i}']
      self._tmp[f"log_{i}"].append(_aux["log"])    
      if (self._k % self._args["traj_iter"]) == 0:
        # update traj
        traj = {"seq":   _aux["seq"]["pseudo"],
                "xyz":   _aux["atom_positions"][:,1,:],
                "plddt": _aux["plddt"],
                "pae":   _aux["pae"]}
        for k,v in traj.items():
          if len(self._tmp[f"traj_{i}"][k]) == self._args["traj_max"]:
            self._tmp[f"traj_{i}"][k].pop(0)
          self._tmp[f"traj_{i}"][k].append(v)
  ...
  def design_test(self, soft_iters=10, temp_iters=10, hard_iters=1,
                    ramp_recycles=True, **kwargs):
    '''three stage design (logits→soft→hard)'''
