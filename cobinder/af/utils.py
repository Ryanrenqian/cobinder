...
from cobinder.shared.protein import _np_kabsch
from cobinder.shared.utils import update_dict, Key
from cobinder.shared.plot import plot_pseudo_3D, make_animation, show_pdb
from cobinder.shared.protein import renum_pdb_str
from cobinder.af.alphafold.common import protein
...
class _af_utils:  
  ...
  def save_pdb(self, filename=None, get_best=True, renum_pdb=True, aux=None):
    ...
    for i in range(self.num_target):
      _aux = aux[f'aux_{i}']["all"]
      
      p = {k:_aux[k] for k in ["aatype","residue_index","atom_positions","atom_mask"]}
      p["b_factors"] = 100 * p["atom_mask"] * _aux["plddt"][...,None]
